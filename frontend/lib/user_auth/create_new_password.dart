...
class _CreateNewPasswordPageState extends State<CreateNewPasswordPage> {
  ...
  Future<void> _onLanjutkanTapped() async {
    ...
    if (password.length < 8) {
      _showSnackBar(
        'Kata sandi terlalu pendek. Harus minimal 8 karakter.',
        const Color(0xFF4A4A4A),
      );
      return;
    }
    if (!RegExp(r'[A-Z]').hasMatch(password)) {
      _showSnackBar(
        'Kata sandi harus memiliki setidaknya satu huruf besar.',
        const Color(0xFF4A4A4A),
      );
      return;
    }
    if (!RegExp(r'[a-z]').hasMatch(password)) {
      _showSnackBar(
        'Kata sandi harus memiliki setidaknya satu huruf kecil.',
        const Color(0xFF4A4A4A),
      );
      return;
    }
    if (!RegExp(r'[0-9]').hasMatch(password)) {
      _showSnackBar(
        'Kata sandi harus memiliki setidaknya satu angka.',
        const Color(0xFF4A4A4A),
      );
      return;
    }
    if (!RegExp(r'[^A-Za-z0-9]').hasMatch(password)) {
      _showSnackBar(
        'Kata sandi harus memiliki setidaknya satu karakter spesial.',
        const Color(0xFF4A4A4A),
      );
      return;
    }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                "Buat kata sandi baru dengan minimal 8 karakter yang juga memuat huruf. Kata sandi ini akan digunakan untuk masuk ke akun kamu",
                style: TextStyle(
                  fontFamily: 'Inter',
                  fontSize: 14,
                  color: Colors.grey,
