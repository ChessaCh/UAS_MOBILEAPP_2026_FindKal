...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _initLocation() async {
    ...
    try {
      ...
      LatLng verifiedLoc = LatLng(pos.latitude, pos.longitude);
      // Fallback untuk emulator yang default lokasinya di Amerika
      if (const Distance().as(LengthUnit.Meter, verifiedLoc, _defaultLocation) >
          10000000) {
        verifiedLoc = _defaultLocation;
      }
      if (mounted) {
        setState(() {
        ...
        setState(() {
          ...
          _places = grouped.entries
              ...
              .map((e) {
                final list = e.value;
                final avgRating =
                    list.fold(0, (s, u) => s + u.rating) / list.length;
                final firstImage =
                    list.expand((u) => u.imagePaths).firstOrNull ?? '';
                return PlaceSummary(
                  placeName: e.key,
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () {
                  ...
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (_) => const SurveyIntroPage()),
                  );
                },
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Colors.white,
      body: SafeArea(
        child: _selectedIndex == 2 ? const ProfilePage() : _buildHomeContent(),
      ),
              ...
              _buildCircularButton(
                ...
                onTap: () {
                  ...
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => const BuatUnggahanPage(),
                    ),
                  );
                },
              ),
              ...
              _buildCircularButton(
                ...
                onTap: () {
                  Navigator.push(
                    context,
                    MaterialPageRoute(
                      builder: (context) => const TripPlanSelectionPage(),
                    ),
                  );
                },
              ),
      ...
      child: Container(
        ...
        child: Center(
          child: imageAsset != null
              ? Image.asset(imageAsset, width: size ?? 26, height: size ?? 26)
