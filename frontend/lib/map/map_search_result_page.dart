...
class _MapSearchResultPageState extends State<MapSearchResultPage> {
  ...
  Future<void> _initAndSearch() async {
    ...
    await _initLocationThenSearch(widget.query);
  }
    ...
    if (permission != LocationPermission.deniedForever &&
      ...
      try {
        ...
        if (mounted) {
          ...
          if (_haversine(loc.latitude, loc.longitude, -6.3001423950968265, 106.6405338994729) > 10000000) {
            loc = const LatLng(-6.3001423950968265, 106.6405338994729);
          }
...
[out:json][timeout:25];
(
  node["name"~"$safeQuery",i](around:50000,${userLoc.latitude},${userLoc.longitude});
  way["name"~"$safeQuery",i](around:50000,${userLoc.latitude},${userLoc.longitude});
  relation["name"~"$safeQuery",i](around:50000,${userLoc.latitude},${userLoc.longitude});
);
out center;
''';
    try {
  ...
  Future<void> _search(String query) async {
    ...
    try {
      ...
      final postMatches = _postSuggestions
          .where((s) => s.label.toLowerCase().contains(trimmed.toLowerCase()))
          .toList();
      for (final p in postMatches) {
        if (p.lat != null && p.lng != null) {
          results.add(_PlaceResult(displayName: p.label, lat: p.lat!, lon: p.lng!));
        }
      }
      ...
      if (_userLocation != null && results.isEmpty) {
